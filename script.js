        ...
        cardCircle.onclick = function () {
          ...
          artist.albums.forEach((album) => {
            ...
            if (!artistIdList.includes(artist.id)) {
              ...
              p.textContent = `Playlists • ${album.tracks.length} song${
              ...
              cardImg.addEventListener("click", () => {
                ...
                var musicItemImg = document.querySelector(
                  ".music_item_img img"
                );
                musicItemImg.src = artist.avatar;
              });
            }
            const rightLayoutQueue = document.querySelector(
              ".right_layout_queue"
            );
            rightLayoutQueue.innerHTML = "";
            album.tracks.forEach((tracks) => {
              playCurrentSong(currentsongIndex);
              playlists.push(tracks.url);
              console.log("Play list from card circle onlick", playlists);
              ...
              trackTitle.textContent = `Title: ${tracks.title}`;

              const trackArtist = document.createElement("p");
              trackArtist.textContent = `Artist: ${artist.name}`;

              const trackImg = document.createElement("img");
              trackImg.src = tracks.img;
              trackImg.alt = tracks.title;
              trackImg.style.width = "50px";
...
function renderCardSquare() {
  fetch("data.json")
    ...
    .then((data) => {
      console.log(data);
      const artistContainerSquareBox = document.getElementById(
        "artistContainerSquareBox"
      );
      ...
      data.artists.forEach((artist) => {
        artist.albums.forEach((album) => {
          album.tracks.forEach((track) => {
            ...
            console.log(
              "allsonglist list beforn event click of card",
              allsongLists
            );
            ...
            card.onclick = function () {
              ...
              playCurrentSong[0];
              console.log("Play list from card onlick", playlists);
            ...
            buttonAdd.onclick = function (event) {
              ...
              if (!isPlayingFromPlaylist) {
                console.log(
                  "ĐÃ RESET PLAYLIST KHI NHẤN THÊM BÀI HÁT VÀO PLAYLIST"
                );
                playlists = [];
                isPlayingFromPlaylist = true;
              }
              console.log("Giá trị của resetPlaylist trước điều kiện:", resetPlaylist);

              // Thêm bài hát vào playlist
              if (resetPlaylist) {
                console.log("Đã reset playlist và xóa hiển thị cũ.");
                const rightLayoutQueue = document.querySelector(
                  ".right_layout_queue"
                );
                rightLayoutQueue.innerHTML = ""; // Xóa nội dung hiển thị playlist
                // playlists = []; // Reset danh sách bài hát trong playlist
                resetPlaylist = false; // Đặt lại trạng thái sau khi reset
              }
              if (!playlists.some((url) => url === track.url)) {
                // Thêm bài hát vào playlist nếu chưa tồn tại
                playlists.push(track.url);
                console.log("Play list from button add", playlists);
                ...
                console.log(
                  `Track "${track.title}" already exists in the playlist.`
                );
              }
            };
          });
